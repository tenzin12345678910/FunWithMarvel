...
//
// let iM = `
// <p>${movieIronMan["title"]}, ${movieIronMan["gross"]}, ${movieIronMan["rottenTomatoes"]}</p>`;
// $("#display-area").append(iM);


for(let i =0;i<movieListDetailed.length; i++){
  let mLD = `<div id="border">
  <img id="ill" src="${movieListDetailed[i]["moviePoster"]}">
  <p>${movieListDetailed[i]["title"]}</p>
  <p>${movieListDetailed[i]["gross"]}</p>
  <p>${movieListDetailed[i]["rottenTomatoes"]}</p>
  <p>
  </div>`
  console.log(mLD);
  $("#display-area").append(mLD)
}
// let mLD = `
// <p>${movieListDetailed["title"]}, ${movieListDetailed["gross"]}, ${movieListDetailed["rottenTomatoes"]}</p>`;
//   $("#display-area").append(mLD);
